...
from .models import SessionLocal, WidgetDB
...
class WidgetService:
    ...
    def get_all_widgets(self) -> List[WidgetResponse]:
        ...
        try:
            ...
            return [
                WidgetResponse.model_validate(widget) for widget in widgets
            ]
        finally:
            db.close()
    ...
    def get_widget_by_id(self, widget_id: int) -> Optional[WidgetResponse]:
        ...
        try:
            db_widget = (
                db.query(WidgetDB).filter(WidgetDB.id == widget_id).first()
            )
            if not db_widget:
                return None
    ...
    def update_widget(
        self, widget_id: int, widget: Widget
